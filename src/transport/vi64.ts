...
/** Largest value a vi64 can carry (2^64 - 1). */
export const VI64_MAX = U64_MAX;

/**
 * Total on-wire length in bytes (1 to 9) of the vi64 beginning with `first`.
 *
 * Only the leading-ones prefix is read, so a caller can size a read before it
 * holds the whole value. Unlike vi64Len, which reports the shortest form of a
 * value, this reports the length of an *encoding* -- non-minimal encodings are
 * legal -- so it is the right function for advancing a cursor over received
 * bytes.
 */
export function vi64PeekLen(first: number): number {
  let n = 1;
  while (n <= 8 && (first & (0x100 >> n)) !== 0) {
    n++;
  }
