...
describe('ISO Box Parsing', () => {
  ...
  test('should parse init segment and extract timescale', () => {
    ...
    // Find the moov box
    const moov = parsed.boxes ? parsed.boxes.find((box: any) => box.type === 'moov') : 
                (parsed.type === 'moov' ? parsed : undefined);
  ...
  test('should parse media segment and extract baseMediaDecodeTime and duration', () => {
    ...
    // Find the moof box
    const moof = parsed.boxes ? parsed.boxes.find((box: any) => box.type === 'moof') : 
                (parsed.type === 'moof' ? parsed : undefined);
