...
export class MediaBuffer {
    ...
    try {
      ...
      if (!(initSegment instanceof ArrayBuffer)) {
        ...
        if (initSegment && initSegment.buffer instanceof ArrayBuffer) {
          ...
          const constructor = initSegment ? initSegment.constructor?.name : 'unknown';
          throw new TypeError(`Init segment must be an ArrayBuffer, but got ${type} (${constructor})`);
        }
      }
  ...
  public parseMediaSegment(mediaSegment: ArrayBuffer | any): MediaTrackInfo {
    try {
      if (!this.initSegment) {
        throw new Error('Init segment must be parsed before parsing media segments');
      }
      ...
      if (!(mediaSegment instanceof ArrayBuffer)) {
        ...
        if (mediaSegment && mediaSegment.buffer instanceof ArrayBuffer) {
          ...
          const constructor = mediaSegment ? mediaSegment.constructor?.name : 'unknown';
          throw new TypeError(`Media segment must be an ArrayBuffer, but got ${type} (${constructor})`);
        }
      }
