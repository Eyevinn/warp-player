...
async function loadReferenceInit(): Promise<Uint8Array> {
  return loadFixture("init.cmaf.mp4");
}
...
async function loadLocmafInitPayload(): Promise<Uint8Array> {
  return parseLocmafObject(await loadLocmafInitObject()).locPayload;
}
...
describe("locmaf reconstruction", () => {
  it("reconstructs an init segment from the locmaf header", async () => {
    const locmafInit = await loadLocmafInitPayload();
  ...
  it("reconstructs a single-sample moof without sampleSizes", async () => {
    const locmafInit = await loadLocmafInitObject();
    const locmafInitPayload = parseLocmafObject(locmafInit).locPayload;
    const referenceInit = await loadReferenceInit();
    const trackMetadata = extractTrackMetadataFromInitSegment(referenceInit);
    decompressLocmafInit(locmafInitPayload, trackMetadata, {
      referenceInitSegment: referenceInit,
    });

    const originalPayload = await loadFullMoofObject(0);
